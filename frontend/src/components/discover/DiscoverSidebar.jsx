...
const DiscoverSidebar = ({ isOpen }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (userContext) {
          // or userContext?.user
          const recommendedClubsRes = await backend.get(
  ...
  return (
    <div
      className={`fixed w-[278px] h-[calc(100vh-80px)] top-[80px] overflow-y-auto left-0 bg-[#4D9FFD] border border-black shadow-[4px_4px_0px_#000000] p-4 transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
    >
      {/* New Events Section */}
      ...
      <div className="flex flex-col items-center">
        <div className="mt-2 ">
          ...
          <div className="flex flex-col gap-3">
            {recommendedClubs
              .slice(0, showRecommendedCardsLimit)
              .map((recommendedClub) => (
                <SidebarCard
