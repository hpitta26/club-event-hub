...
def student_verify_email(request, token):
    try:
        ...
        return Response(status=205) # gracefully error
    except Exception as e:
        print(e) 
        return Response({"Error": "Catastrophic error!!"}, status=500)
...
def student_logout(request):
    try:
        ...
        response = Response({"status": "error", "message": "Server error!"}, status=500)
        response.delete_cookie('csrfToken')
        return response